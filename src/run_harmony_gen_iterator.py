...
def run_harmony_gen(method,file,weights,weights_data,hard_constraints_choice,time_limit,input_melody,music_index):
    ...
    # Importing melody
    if input_melody[-3:] == 'csv':
        logger.info('Melody is .csv')
        musical_work_df = pd.read_csv(input_melody)
        musical_corpus = []
    ...
    elif input_melody[-3:] == 'mid':
        logger.info('Melody is .mid')
        input_midi = input_melody
        ...
        filename = str.split(str.split(input_melody,'/')[-1],'_')
    ...
    hard_constraint_choices = list(pd.read_csv(hard_constraints_choice).columns)
    ...
    # define weights
    if weights == 'defined':
        weight_df = pd.read_csv(weights_data)
    ...
    #%%
    logger.info(f'==============================================')
    logger.info(f'Generating Harmony with {method} model')
    
    # get start time
    start = timeit.default_timer()
    
    music = musical_corpus[music_index]
